...
def debug_rmb_issue():
    ...
    try:
        ...
        print("\n📊 RMB帳戶餘額:")
        cursor.execute("""
            SELECT id, name, balance, currency, holder_id
            FROM cash_accounts 
        ...
        for account in accounts:
            account_id, name, balance, currency, holder_id = account
            print(f"  ID: {account_id}, 名稱: {name}, 餘額: {balance}, 幣別: {currency}, 持有人ID: {holder_id}")
        ...
        if abs(difference) > 0.01:
            ...
            if entries:
                print(f"  最近20筆RMB流水記錄:")
                for entry in entries:
                    entry_id, account_id, rmb_change, twd_change, description, entry_type, entry_date = entry
                    print(f"    ID: {entry_id}, 帳戶ID: {account_id}, RMB變動: {rmb_change}, 類型: {entry_type}")
                    print(f"      描述: {description}, 日期: {entry_date}")
            else:
                print("  沒有RMB流水記錄")
        
        # 6. 檢查是否有被標記為非活躍的RMB帳戶
        print(f"\n🔍 檢查非活躍的RMB帳戶:")
        cursor.execute("""
            SELECT id, name, balance, currency, holder_id, is_active
            FROM cash_accounts 
            WHERE currency = 'RMB' AND is_active = 0
            ORDER BY id
        """)
        inactive_accounts = cursor.fetchall()
        
        if inactive_accounts:
            print(f"  找到 {len(inactive_accounts)} 個非活躍的RMB帳戶:")
