...
# ===================================================================
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'a_very_very_secret_key_that_is_long_and_secure')
basedir = os.path.abspath(os.path.dirname(__file__))
instance_path = os.path.join(basedir, 'instance')
os.makedirs(instance_path, exist_ok=True)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(instance_path, 'sales_system_v4.db') # 使用新檔名以示區別
...
class User(db.Model, UserMixin):
    ...
    role = db.Column(db.String(50), default='operator', nullable=False) # <---【修正】引入 Role
...
class Holder(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    cash_accounts = db.relationship('CashAccount', back_populates='holder', lazy='select', cascade="all, delete-orphan")
...
class Customer(db.Model):
    ...
    sales = db.relationship('SalesRecord', back_populates='customer', lazy='dynamic', cascade="all, delete-orphan")
...
class Channel(db.Model):
    ...
    name = db.Column(db.String(100), unique=True, nullable=False)
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    purchase_records = db.relationship('PurchaseRecord', back_populates='channel', lazy='dynamic')
...
class PurchaseRecord(db.Model):
    ...
    operator_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False) # <---【修正】統一外鍵目標
    channel = db.relationship('Channel', back_populates='purchase_records')
    payment_account = db.relationship('CashAccount', foreign_keys=[payment_account_id], backref="paid_purchases")
    deposit_account = db.relationship('CashAccount', foreign_keys=[deposit_account_id], backref="deposited_purchases")
    operator = db.relationship('User', backref="purchase_records")

class SalesRecord(db.Model):
    __tablename__ = 'sales_records'
    id = db.Column(db.Integer, primary_key=True)
    customer_id = db.Column(db.Integer, db.ForeignKey('customers.id'), nullable=False) # <---【修正】指向 customers 表
    ...
    is_settled = db.Column(db.Boolean, nullable=False, default=False) # <---【修正】使用 is_settled
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    operator_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False) # <---【修正】統一外鍵目標
    customer = db.relationship('Customer', back_populates='sales')
    operator = db.relationship('User', backref="sales_records")
    transactions = db.relationship('Transaction', back_populates='sales_record', cascade="all, delete-orphan")
...
class LedgerEntry(db.Model):
    ...
    account_id = db.Column(db.Integer, db.ForeignKey('cash_accounts.id'), nullable=True) # <---【修正】允許為空
...
def index():
    if current_user.is_authenticated:
        return redirect(url_for('admin_dashboard' if current_user.is_admin else 'cash_management'))
    return redirect(url_for('login'))
...
def admin_dashboard():
    """儀表板頁面"""
    try:
        total_twd_cash = db.session.execute(db.select(func.sum(CashAccount.balance)).filter(CashAccount.currency == 'TWD')).scalar() or 0.0
        total_rmb_stock = db.session.execute(db.select(func.sum(CashAccount.balance)).filter(CashAccount.currency == 'RMB')).scalar() or 0.0
        
        latest_purchase = db.session.execute(db.select(PurchaseRecord).order_by(PurchaseRecord.purchase_date.desc())).scalars().first()
        ...
        total_unsettled_amount_twd = db.session.execute(db.select(func.sum(Customer.total_receivables_twd))).scalar() or 0.0
        chart_data = {'labels': ['台幣現金', '人民幣庫存(估值)'], 'values': [total_twd_cash, (total_rmb_stock * current_buy_rate)]}
...
def sales_entry():
    ...
    try:
        ...
        holders_with_rmb_accounts = db.session.execute(
            db.select(Holder).filter_by(is_active=True).options(
                db.selectinload(Holder.cash_accounts)
            )
        ).scalars().all()

        owner_rmb_accounts_grouped = []
        for holder in holders_with_rmb_accounts:
            rmb_accs = [acc for acc in holder.cash_accounts if acc.currency == 'RMB' and acc.is_active]
            if rmb_accs:
                owner_rmb_accounts_grouped.append({'holder_name': holder.name, 'accounts': rmb_accs})

        # 3. 查詢最近 10 筆未結清 (is_settled = False) 的銷售紀錄
        recent_unsettled_sales = db.session.execute(
            db.select(SalesRecord).filter_by(is_settled=False).order_by(SalesRecord.created_at.desc()).limit(10)
        ).scalars().all()
...
def api_sales_entry():
    ...
    try:
        ...
        if not all([customer_id, rmb_account_id, rmb_amount > 0, exchange_rate > 0]):
            return jsonify({'status': 'error', 'message': '客戶、出貨帳戶和金額都必須正確填寫。'}), 400
        ...
        if rmb_account.balance < rmb_amount:
            return jsonify({'status': 'error', 'message': f'RMB 庫存不足！帳戶 {rmb_account.name} 僅剩 {rmb_account.balance:,.2f}。'}), 400
...
def buy_in():
    ...
    try:
        ...
        holders_with_accounts = db.session.execute(
            db.select(Holder).filter_by(is_active=True).options(
                db.selectinload(Holder.cash_accounts)
            )
        ).scalars().all()
        ...
        for holder in holders_with_accounts:
            twd_accs = [acc for acc in holder.cash_accounts if acc.currency == 'TWD' and acc.is_active]
            rmb_accs = [acc for acc in holder.cash_accounts if acc.currency == 'RMB' and acc.is_active]
            
            if twd_accs:
                owner_twd_accounts_grouped.append({'holder_name': holder.name, 'accounts': twd_accs})
            if rmb_accs:
                owner_rmb_accounts_grouped.append({'holder_name': holder.name, 'accounts': rmb_accs})

        recent_purchases = db.session.execute(
            db.select(PurchaseRecord).order_by(PurchaseRecord.purchase_date.desc()).limit(10)
        ).scalars().all()
    ...
    try:
        ...
        if action == 'record_purchase':
            ...
            if not all([payment_account_id, deposit_account_id, rmb_amount > 0, exchange_rate > 0]):
                return jsonify({'status': 'error', 'message': '所有帳戶和金額欄位都必須正確填寫。'}), 400
            ...
            if not deposit_account or deposit_account.currency != 'RMB':
                return jsonify({'status': 'error', 'message': '無效的 RMB 入庫帳戶。'}), 400
            ...
            if not final_channel_id and channel_name_manual:
                channel = db.session.execute(db.select(Channel).filter_by(name=channel_name_manual)).scalar_one_or_none()
                if not channel:
            ...
            return jsonify({'status': 'success', 'message': f'交易成功！已從 {payment_account.name} 付款，並將 RMB 存入 {deposit_account.name}。'})
...
def api_add_user():
    ...
    if len(password) < 4: # 增加一個簡單的密碼長度檢查
        return jsonify({'status': 'error', 'message': '為安全起見，密碼長度至少需要 4 個字元。'}), 400
    
    if db.session.execute(db.select(User).filter_by(username=username)).scalar_one_or_none():
        return jsonify({'status': 'error', 'message': f'用戶名 "{username}" 已經被註冊。'}), 409 # 409 Conflict
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': f'使用者 "{username}" 已成功創建！'})

    except Exception as e:
        db.session.rollback()
        print(f"!!! Error in api_add_user: {e}")
        return jsonify({'status': 'error', 'message': '伺服器內部錯誤，新增失敗。'}), 500
