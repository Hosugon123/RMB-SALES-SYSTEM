...
class Transaction(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False) # '待收款', '已入帳', '已完成' 等
    rmb_amount = db.Column(db.Float, nullable=False)
    twd_amount = db.Column(db.Float, nullable=False)
    buy_rate = db.Column(db.Float) # 買入時的匯率
    sell_rate = db.Column(db.Float) # 賣出時的匯率
    order_time = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    source_channel_name = db.Column(db.String(100)) # 買入時的來源渠道
    customer_id = db.Column(db.Integer, db.ForeignKey('user.id')) # 賣出時的客戶ID
    operator_name = db.Column(db.String(80)) # 操作者名稱
...
# 資金調撥紀錄表
class FundMovement(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    from_account_name = db.Column(db.String(100), nullable=False)
    to_account_name = db.Column(db.String(100), nullable=False)
    amount = db.Column(db.Float, nullable=False)
    currency = db.Column(db.String(10), nullable=False) # 'RMB' or 'TWD'
    move_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
...
class CardPurchase(db.Model):
    ...
    rmb_received = db.Column(db.Float, nullable=False)
    twd_amount = db.Column(db.Float, nullable=False)
    twd_rate_effective = db.Column(db.Float) # 實際匯率
    cashback_amount = db.Column(db.Float, default=0)
...
# ===================================================================
#           用下面的函式，完整替換掉您舊的 init_db 函式
# ===================================================================
def init_db():
    """
    清除現有資料並使用原生 SQL 建立一個全新的、與應用邏輯匹配的資料庫結構。
    """
    # 這裡我們直接獲取資料庫檔案路徑，而不是透過 g
    db_path = app.config['DATABASE']
    db_dir = os.path.dirname(db_path)
    if not os.path.exists(db_dir):
        os.makedirs(db_dir)

    # 連接到資料庫 (如果檔案不存在，會自動建立)
    ...
    # (如果還有其他表，也可以在這裡加入 DROP TABLE 指令)

    # --- 建立與您 ORM 邏輯最匹配的核心資料表 ---

    # 1. users 表：用於儲存管理員、客戶和渠道
    cursor.execute('''
        CREATE TABLE users (
    ...
    # 2. transactions 表：核心交易紀錄表
    cursor.execute('''
        CREATE TABLE transactions (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            transaction_type TEXT NOT NULL,         -- 'buy' 或 'sell'
            status TEXT NOT NULL,                   -- '待收款', '已入帳' 等
            rmb_amount REAL NOT NULL,
            twd_amount REAL NOT NULL,
            buy_rate REAL,                          -- 買入時的匯率
            sell_rate REAL,                         -- 賣出時的匯率
            order_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            source_channel_name TEXT,               -- 買入時的來源渠道名稱 (直接儲存字串)
            customer_id INTEGER,                    -- 賣出時的客戶ID
            operator_name TEXT,                     -- 操作者名稱
            note TEXT,
    ...
    # 建立預設的管理員帳號
    admin_password_hash = generate_password_hash('asdf1234555', method='pbkdf2:sha256')
    cursor.execute(
...
def get_db():
    ...
    # 如果不是，那肯定就是在「我們自家的廚房 (Codespaces)」裡了
    # 我們就把食材放在廚房裡一個叫做「櫃子 (`instance`)」的地方
    # (如果櫃子不存在，就先把它造出來)
...
def query_db(query, args=(), one=False):
    """
    執行資料庫查詢，並返回結果。
    - query: SQL 查詢語句 (使用 '?' 作為佔位符)
    - args: 查詢參數的元組
    - one: 如果為 True，則只返回第一條結果，否則返回所有結果
    """
    # 透過 get_db() 獲取當前請求的資料庫連線
    # 我們的 get_db() 函式已經設定好了 row_factory，所以這裡不需要再設定
    db = get_db() 
    ...
    # 對於 INSERT/UPDATE/DELETE，我們會在路由函式中手動 commit，
    # 這裡的 query_db 只專注於查詢，所以不需要 commit。
    
    if one:
        # 如果指定 one=True，只返回第一筆，如果沒有結果則返回 None
