...
# --- App Config ---
# 設定一個複雜的密鑰，用於保護 session
app.config['SECRET_KEY'] = 'change-this-to-a-long-random-string-in-production'

# 設定資料庫檔案的路徑 (兼容本地/Codespaces)
...
class User(UserMixin, db.Model):
    ...
    password_hash = db.Column(db.String(256)) # 客戶/渠道可以沒有密碼
...
# ===================================================================
# 4. 資料庫初始化指令 (全新 ORM 版本)
# ===================================================================
@app.cli.command('init-db')
def init_db_command():
    """清除現有資料並根據 Class 模型建立新表格。"""
...
@admin_required
def admin_dashboard():
    # 這裡的邏輯需要用 ORM 重寫，我們先提供一個能運作的基礎版本
    # 未來可以根據 Transaction 表來計算這些指標
    total_assets_twd = 0 
...
# (其他頁面的路由，如 card_accounting, sales_entry, transactions 等，可以後續再逐一加入)
# (我們先確保買入頁面的核心功能可以運作)
...
def add_purchase_channel_ajax():
    ...
    if existing:
        if not existing.is_active: # 如果存在但不活躍，則恢復
